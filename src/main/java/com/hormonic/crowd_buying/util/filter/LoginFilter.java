...
public class LoginFilter extends UsernamePasswordAuthenticationFilter {
    ...
    private final long VALID_TIME = 60 * 30 * 1000L;  // 30분
    ...
    public Authentication attemptAuthentication(HttpServletRequest req, HttpServletResponse res) throws AuthenticationException {
        ...
        // parameter name을 개별적으로 설정한 userId/userPw 가 아니라 username/password로 설정해서 보내야 한다.
        // 1. 로그인 정보를 폼데이터로 받는 경우
        /*
        String userId = obtainUsername(req);
        String userPw = obtainPassword(req);
        */

        // 2. 로그인 정보를 JSON 형태로 받는 경우
